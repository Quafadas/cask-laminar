  ...
  val tests = Tests{
     test("with database"){ 
        ...
        test("that we can create a new todo") {
          ...
          val req = requests.put(s"$host/${TodoRoutes.newTodo.route}", data= write(newTodo))          
        ...
        test("that we can update our todo") {
          val theCopy = withId.get.copy(completed = true, description = "modify a todo")          
          val req3 = requests.post(s"$host/${TodoRoutes.updateTodo.route}", data = write(theCopy))
