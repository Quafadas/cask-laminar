...
trait ServerT extends cask.Routes {
  ...
  def typeCheck1 = routeTypeCheck(allToDos(), TodoRoutes.listAllTodos.decode(""))
  ...
  def t3 = routeTypeCheck(createToDos(cask.Request(null, Seq(""))), TodoRoutes.newTodo.decode(""))
  ...
  def t4 = routeTypeCheck(updateTodo(cask.Request(null, Seq(""))), TodoRoutes.updateTodo.decode(""))
  ...
  //----------------  
  val sillySuggestions: Seq[String] = Seq("apple", "pear", "orange", "strawberries", "orangutan")
  ...
  def t6 = routeTypeCheck(suggest(cask.Request(null, Seq(""))), SuggestionRoutes.filterSuggestions.decode(""))
