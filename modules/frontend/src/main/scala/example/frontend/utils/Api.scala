...
trait RouteApiT {
  ...
  * 
  */
  def simpleRoute[T, D](route: Route[T, D],
    data: D,
    timeoutMs: Int = 0,
   ...
   def pathSegmentedRoute[T, D](route: Route[T, D],
    data: D,
    timeoutMs: Int = 0,
    headers: Map[String, String] = Map.empty,
    withCredentials: Boolean = false,
  ...
  )(replacePathSegemnts: String => String)(implicit w : Writer[D])
   : EventStream[T] = {    
    val newRoute = route.copy(route = replacePathSegemnts(route.route))
    simpleRoute(newRoute,
    data,
    timeoutMs,
    headers,
    withCredentials,
  ...
  object RouteApi extends RouteApiT {
    ...
    def simpleRouteNoObserverStream[T, D](route: Route[T, D]): EventStream[T] = {
      ...
      AjaxEventStream.sendRequest(request, route.method.toUpperCase(), route.route)
      stream
    }

/*     def simpleRouteNoObserver[T, D](route: Route[T, D]): T = {
    ...
    override def simpleRoute[T, D](route: Route[T, D],
      data: D = None,
      timeoutMs: Int = 0,
      headers: Map[String, String] = Map.empty,
      withCredentials: Boolean = false,
